...
def england_pull_fixture_data(date,**kwargs):
    import subprocess
    import re
    ti = kwargs['ti']
...
england_pull_fixture_data_task = PythonOperator(
    task_id='england.pull.fixture.data.task',
    python_callable=england_pull_fixture_data,
    op_args=[formatted_previous_day],
    provide_context=True,
    dag=dag
)
def england_pull_injuries_data(date,**kwargs):
    import subprocess
    import re
    ti = kwargs['ti']
    ...
    for i in pulled_value:
        command = f'curl "http://{host_fastapi}:{port_fastapi}/england/injuries?id={i}&date={date}"'
        result = subprocess.run(command, shell=True, capture_output=True, text=True)
        print(result)
england_pull_injuries_data_task = PythonOperator(
    task_id='england.pull.injuries.data.task',
    python_callable=england_pull_injuries_data,
    op_args=[formatted_previous_day],
    provide_context=True,
    dag=dag
)
end_task = EmptyOperator(
    task_id = 'end',
    dag = dag)
start_task >> england_curl_and_push_task
england_curl_and_push_task >> [england_pull_fixture_data_task,england_pull_injuries_data_task]
[england_pull_fixture_data_task,england_pull_injuries_data_task] >> end_taskr

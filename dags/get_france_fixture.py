...
def france_pull_fixture_data(date,**kwargs):
    import subprocess
    import re
    ti = kwargs['ti']
...
france_pull_fixture_data_task = PythonOperator(
    task_id='france.pull.fixture.data.task',
    python_callable=france_pull_fixture_data,
    op_args=[formatted_previous_day],
    provide_context=True,
    dag=dag
)
def france_pull_injuries_data(date,**kwargs):
    import subprocess
    import re
    ti = kwargs['ti']
    ...
    for i in pulled_value:
        command = f'curl "http://{host_fastapi}:{port_fastapi}/france/injuries?id={i}&date={date}"'
        result = subprocess.run(command, shell=True, capture_output=True, text=True)
        print(result)
france_pull_injuries_data_task = PythonOperator(
    task_id='france.pull.injuries.data.task',
    python_callable=france_pull_injuries_data,
    op_args=[formatted_previous_day],
    provide_context=True,
    dag=dag
)
end_task = EmptyOperator(
    task_id = 'end',
    dag = dag)
start_task >> france_curl_and_push_task
france_curl_and_push_task >> [france_pull_fixture_data_task,france_pull_injuries_data_task]
[france_pull_fixture_data_task,france_pull_injuries_data_task] >> end_task
